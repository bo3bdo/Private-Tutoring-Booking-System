                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    @if($supportTicket->category)
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ ucfirst($supportTicket->category) }}</p>
                        </div>
                    @endif
                            ...
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($reply->is_internal)
                                        <span class="text-xs bg-gray-200 text-gray-700 px-2 py-0.5 rounded">Internal</span>
                                    @endif
                                </div>
            ...
            @if(!$supportTicket->isClosed())
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
                    ...
                    <form method="POST" action="{{ route('student.support-tickets.reply', $supportTicket) }}">
                        @csrf
                        <div class="mb-4">
                            <textarea name="message" rows="4" required class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 transition resize-none" placeholder="Type your reply..."></textarea>
                        </div>
