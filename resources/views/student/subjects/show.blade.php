<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('student.subjects.index') }}" class="text-blue-600 hover:text-blue-800">{{ __('common.Subjects') }}</a> / {{ $subject->name }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-lg font-semibold text-gray-800">{{ __('common.Recorded Courses') }}</h2>
                    ...
                    <p class="text-sm text-gray-600 mb-4">{{ __('common.Explore recorded courses for') }} {{ $subject->name }}{{ __('common.. Learn at your own pace with video lessons from expert teachers.') }}</p>
                </div>
            </div>

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h2 class="text-lg font-semibold text-gray-800 mb-4">{{ __('common.Available Teachers') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @if($reviewsCount > 0)
                                            <div class="flex items-center gap-2 mt-2">
                                                ...
                                                @if($reviewsCount > 0)
                                                    <button type="button" onclick="openReviewsModal({{ $teacher->id }})" class="text-xs text-gray-500 hover:text-blue-600 cursor-pointer underline">
                                                        ({{ $reviewsCount }} {{ $reviewsCount === 1 ? __('common.review') : __('common.reviews') }})
                                                    </button>
                                                @else
                                                    <span class="text-xs text-gray-500">({{ $reviewsCount }} {{ $reviewsCount === 1 ? __('common.review') : __('common.reviews') }})</span>
                                                @endif
                                            </div>
                                        @else
                                            <p class="text-xs text-gray-500 mt-2">{{ __('common.No reviews yet') }}</p>
                                        @endif
                                    </div>
                                </div>
                                @if($teacher->bio)
                                ...
                                <div class="mt-4 flex items-center gap-2 flex-wrap">
                                    @if($teacher->supports_online)
                                        <span class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-800">{{ __('common.Online') }}</span>
                                    @endif
                                    @if($teacher->supports_in_person)
                                        <span class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-800">{{ __('common.In Person') }}</span>
                                    @endif
                        ...
                        @empty
                            <div class="col-span-full text-center py-12">
                                ...
                                <p class="text-slate-600 text-lg font-semibold">{{ __('common.No teachers available') }}</p>
                                <p class="text-slate-500 text-sm mt-1">{{ __('common.No teachers are currently available for this subject.') }}</p>
                            </div>
                        @endforelse
                    </div>
                </div>
    ...
    <!-- Reviews Modal -->
    <div id="reviewsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div class="p-6 border-b border-slate-200 flex items-center justify-between">
                <div>
                    <h3 class="text-xl font-bold text-gray-900" id="modalTeacherName">{{ __('common.Teacher Reviews') }}</h3>
                    <p class="text-sm text-gray-600 mt-1">
                        <span id="modalAverageRating"></span> · <span id="modalReviewsCount"></span> {{ __('common.reviews') }}
                    </p>
                </div>
                <button type="button" onclick="closeReviewsModal()" class="text-gray-400 hover:text-gray-600 transition">
    ...
    <script>
        function openReviewsModal(teacherId) {
            ...
            // Show loading
            content.innerHTML = '<div class="flex items-center justify-center py-12"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div></div>';
            ...
            fetch(`/student/teachers/${teacherId}/reviews`)
                ...
                .then(data => {
                    ...
                    if (data.reviews.length === 0) {
                        content.innerHTML = '<div class="text-center py-12"><p class="text-gray-500">{{ __('common.No reviews yet') }}</p></div>';
                        return;
                    }
                            ...
                            <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                                <div class="flex items-start justify-between mb-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-semibold text-gray-900">${review.user_name}</span>
                                    </div>
                ...
                .catch(error => {
                    ...
                    content.innerHTML = '<div class="text-center py-12"><p class="text-red-500">{{ __('common.Error loading reviews') }}</p></div>';
                });
        }
