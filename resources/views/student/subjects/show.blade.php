    ...
    <div id="reviewsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div class="p-6 border-b border-slate-200 flex items-center justify-between">
                <div>
                    <h3 class="text-xl font-bold text-gray-900" id="modalTeacherName">Teacher Reviews</h3>
                    <p class="text-sm text-gray-600 mt-1">
                        <span id="modalAverageRating"></span> · <span id="modalReviewsCount"></span> reviews
                    </p>
                </div>
    ...
    <script>
        function openReviewsModal(teacherId) {
            ...
            fetch(`/student/teachers/${teacherId}/reviews`)
                ...
                .then(data => {
                    document.getElementById('modalTeacherName').textContent = data.teacher.name + ' Reviews';
                    document.getElementById('modalAverageRating').textContent = Math.round(data.teacher.average_rating) + ' stars';
                    document.getElementById('modalReviewsCount').textContent = data.teacher.reviews_count;

                    if (data.reviews.length === 0) {
                        content.innerHTML = '<div class="text-center py-12"><p class="text-gray-500">No reviews yet</p></div>';
                        return;
                    }
                ...
                .catch(error => {
                    ...
                    content.innerHTML = '<div class="text-center py-12"><p class="text-red-500">Error loading reviews</p></div>';
                });
        }
