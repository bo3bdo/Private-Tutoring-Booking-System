    ...
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ $subject->name }} - {{ __('common.Recorded Courses') }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($courses->isEmpty())
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-12 text-center">
                    ...
                    <p class="text-gray-500 mb-2">{{ __('common.No courses available for this subject yet.') }}</p>
                    <a href="{{ route('student.subjects.show', $subject) }}" class="text-blue-600 hover:text-blue-800 font-semibold">{{ __('common.Back to Subject') }}</a>
                </div>
            @else
