<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">{{ __('common.Chat with your teachers') }}</p>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($conversations->isEmpty())
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-12 text-center">
                    ...
                    <p class="text-gray-600 text-lg mb-2">{{ __('common.No conversations yet') }}</p>
                    <p class="text-gray-500 text-sm">{{ __('common.Start a conversation with your teacher from a booking page') }}</p>
                </div>
            @else
