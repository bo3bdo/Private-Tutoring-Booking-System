        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 overflow-hidden">
                <div class="p-6 md:p-8">
                    <div class="mb-6">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">{{ __('common.User Information') }}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('common.Update user account details and role') }}</p>
                    </div>

                    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6" x-data="{ role: '{{ old('role', $currentRole) }}' }">
                        @csrf
                        @method('PUT')

                        <!-- Basic Information -->
                        <div class="space-y-4">
                            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">{{ __('common.Basic Information') }}</h4>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required
                                        class="w-full pl-12 rounded-xl border-2 border-slate-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-3 text-sm placeholder-gray-400 focus:border-slate-400 dark:focus:border-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:focus:ring-gray-500 focus:ring-offset-2 transition @error('email') border-red-500 @enderror"
                            ...
                            <div>
                                ...
                                <select name="role" id="role" x-model="role" required
                                    class="w-full rounded-xl border-2 border-slate-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-3 text-sm focus:border-slate-400 dark:focus:border-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:focus:ring-gray-500 focus:ring-offset-2 transition @error('role') border-red-500 @enderror">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="password" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">
                                        {{ __('common.New Password') }} <span class="text-gray-400 dark:text-gray-500 font-normal">({{ __('common.Leave blank to keep current') }})</span>
                                    </label>
                        ...
                        <div x-show="role === 'teacher'" x-transition class="space-y-4 pt-6 border-t border-slate-200 dark:border-gray-700">
                            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">{{ __('common.Teacher Profile') }}</h4>
                            
                            <div>
                                <label for="bio" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">
                                    {{ __('common.Bio') }} <span class="text-gray-400 dark:text-gray-500 font-normal">({{ __('common.Optional') }})</span>
                                </label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="hourly_rate" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">
                                        {{ __('common.Hourly Rate') }} (BHD) <span class="text-gray-400 dark:text-gray-500 font-normal">({{ __('common.Optional') }})</span>
                                    </label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        ...
                                        <input type="number" name="hourly_rate" id="hourly_rate" value="{{ old('hourly_rate', $user->teacherProfile?->hourly_rate ?? 0) }}" step="0.01" min="0"
                                            class="w-full pl-16 rounded-xl border-2 border-slate-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-3 text-sm placeholder-gray-400 focus:border-slate-400 dark:focus:border-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:focus:ring-gray-500 focus:ring-offset-2 transition @error('hourly_rate') border-red-500 @enderror"
                                            placeholder="25.00">
                                    </div>
                                    @error('hourly_rate')
                                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                ...
                                <div>
                                    <label for="default_location_id" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">
                                        {{ __('common.Default Location') }} <span class="text-gray-400 dark:text-gray-500 font-normal">({{ __('common.Optional') }})</span>
                                    </label>
                            ...
                            <div>
                                <label for="default_meeting_provider" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">
                                    {{ __('common.Default Meeting Provider') }} <span class="text-gray-400 dark:text-gray-500 font-normal">({{ __('common.Optional') }})</span>
                                </label>
                        ...
                        <div x-show="role === 'student'" x-transition class="space-y-4 pt-6 border-t border-slate-200 dark:border-gray-700">
                            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">{{ __('common.Student Profile') }}</h4>
                            
                            <div>
                                <label for="phone" class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">
                                    {{ __('common.Phone Number') }} <span class="text-gray-400 dark:text-gray-500 font-normal">({{ __('common.Optional') }})</span>
                                </label>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-200 dark:border-gray-700">
                            <a href="{{ route('admin.users.index') }}" class="inline-flex items-center justify-center px-6 py-3 border-2 border-slate-300 dark:border-gray-600 rounded-xl text-sm font-semibold text-slate-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-slate-50 dark:hover:bg-gray-600 hover:border-slate-400 dark:hover:border-gray-500 transition">
                                {{ __('common.Cancel') }}
                            </a>
