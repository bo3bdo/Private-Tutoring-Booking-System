<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">{{ __('common.Manage teachers and their profiles') }}</p>
            </div>
            <a href="{{ route('admin.teachers.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl text-sm font-semibold text-white shadow-lg hover:from-slate-800 hover:to-slate-700 transform hover:scale-105 transition">
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($errors->has('error'))
                <div class="mb-4 bg-red-50 border-l-4 border-red-400 rounded-xl p-4 shadow-sm">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('common.Hourly Rate') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('common.Subjects') }}</th>
                                    ...
                                    <tr class="hover:bg-gray-50 transition">
                                            ...
                                            @if($teacher->subjects->isNotEmpty())
                                                <div class="flex flex-wrap gap-1">
                                                    ...
                                                    @if($teacher->subjects->count() > 3)
                                                        <span class="text-xs text-gray-500">+{{ $teacher->subjects->count() - 3 }}</span>
                                                    @endif
                                                </div>
                                            @else
                                                <span class="text-gray-400">{{ __('common.No subjects') }}</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.teachers.show', $teacher) }}" class="text-blue-600 hover:text-blue-900 font-semibold">{{ __('common.View') }}</a>
                                                <a href="{{ route('admin.teachers.edit', $teacher) }}" class="text-indigo-600 hover:text-indigo-900 font-semibold">{{ __('common.Edit') }}</a>
                                    ...
                                    <tr>
                                        <td colspan="6" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="text-gray-500 mb-2">No teachers found.</p>
                                                <a href="{{ route('admin.teachers.create') }}" class="text-blue-600 hover:text-blue-800 font-semibold">Add one</a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
