    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            <div class="min-w-0 flex-1">
                ...
                <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('common.Manage teachers and their profiles') }}</p>
            </div>
            <a href="{{ route('admin.teachers.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-slate-900 to-slate-800 dark:from-slate-700 dark:to-slate-600 rounded-lg sm:rounded-xl text-sm font-semibold text-white shadow-lg hover:from-slate-800 hover:to-slate-700 dark:hover:from-slate-600 dark:hover:to-slate-500 transform hover:scale-105 transition">
                        ...
                        <div class="p-4">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex items-center gap-3 min-w-0 flex-1">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $teacher->user->name }}</h3>
                                        <p class="text-xs text-gray-600 dark:text-gray-400 truncate mt-0.5">{{ $teacher->user->email }}</p>
                                    </div>
                                </div>
                            ...
                            <div class="space-y-2 mb-3">
                                <div class="flex items-center justify-between text-xs">
                                    <span class="text-gray-500 dark:text-gray-400">{{ __('common.Hourly Rate') }}:</span>
                                    <span class="text-gray-900 dark:text-white font-semibold">{{ $teacher->hourly_rate ? number_format($teacher->hourly_rate, 2).' BHD' : 'N/A' }}</span>
                                </div>
                                @if($teacher->subjects->isNotEmpty())
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        @if($teacher->subjects->count() > 2)
                                            <span class="text-xs text-gray-500 dark:text-gray-400">+{{ $teacher->subjects->count() - 2 }}</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
                            <div class="flex items-center gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                                <a href="{{ route('admin.teachers.show', $teacher) }}" class="flex-1 text-center px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition">
                                    {{ __('common.View') }}
                                </a>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.Teacher') }}</th>
                                    ...
                                    <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.Hourly Rate') }}</th>
                                    <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider hidden lg:table-cell">{{ __('common.Subjects') }}</th>
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                        <td class="px-4 sm:px-6 py-3 sm:py-4">
                                            <div class="flex items-center gap-2 sm:gap-3">
                                                <div class="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 dark:bg-blue-900/50 rounded-lg flex items-center justify-center">
                                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                                    </svg>
                                                </div>
                                                <div class="min-w-0 flex-1">
                                                    <div class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $teacher->user->name }}</div>
                                                    <div class="text-xs text-gray-600 dark:text-gray-400 truncate mt-0.5 md:hidden">{{ $teacher->user->email }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 hidden md:table-cell text-sm text-gray-600 dark:text-gray-400 truncate max-w-xs">{{ $teacher->user->email }}</td>
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 text-sm text-gray-900 dark:text-white">
                                        ...
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 hidden lg:table-cell text-sm text-gray-600 dark:text-gray-400">
                                            ...
                                            @else
                                                <span class="text-gray-400 dark:text-gray-500 text-xs">{{ __('common.No subjects') }}</span>
                                            @endif
                                        </td>
                                        <td class="px-4 sm:px-6 py-3 sm:py-4">
