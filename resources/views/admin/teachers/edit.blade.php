        ...
        <div class="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div class="min-w-0 flex-1">
                <h2 class="font-semibold text-lg sm:text-xl text-gray-800 dark:text-white leading-tight truncate">
                    {{ __('common.Edit') }}: {{ $teacher->user->name }}
                </h2>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 overflow-hidden">
                <div class="p-4 sm:p-6 md:p-8">
                    <div class="mb-4 sm:mb-6">
                        ...
                        <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Update teacher account details</p>
                    </div>

                    <form method="POST" action="{{ route('admin.teachers.update', $teacher) }}" class="space-y-4 sm:space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-4 pt-6 border-t border-slate-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" name="hourly_rate" id="hourly_rate" value="{{ old('hourly_rate', $teacher->hourly_rate) }}" step="0.01" min="0"
                            ...
                            <div>
                                ...
                                <select name="default_meeting_provider" id="default_meeting_provider"
                                    ...
                                    <option value="none" {{ old('default_meeting_provider', $teacher->default_meeting_provider->value) == 'none' ? 'selected' : '' }}>None</option>
                                    <option value="custom" {{ old('default_meeting_provider', $teacher->default_meeting_provider->value) == 'custom' ? 'selected' : '' }}>Custom URL</option>
                                    <option value="zoom" {{ old('default_meeting_provider', $teacher->default_meeting_provider->value) == 'zoom' ? 'selected' : '' }}>Zoom</option>
                                    <option value="google_meet" {{ old('default_meeting_provider', $teacher->default_meeting_provider->value) == 'google_meet' ? 'selected' : '' }}>Google Meet</option>
                                </select>
