    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            <div class="min-w-0 flex-1">
                ...
                <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('common.View and manage all bookings') }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-lg border border-slate-200 dark:border-gray-700 p-3 sm:p-4">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $stats['upcoming'] }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-lg border border-slate-200 dark:border-gray-700 p-3 sm:p-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-lg border border-slate-200 dark:border-gray-700 p-3 sm:p-4">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-red-600 dark:text-red-400">{{ $stats['cancelled'] }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 p-3 sm:p-4">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('admin.bookings.index') }}" class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-semibold {{ !request('filter') ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' }}">
                        {{ __('common.All') }}
                    </a>
                    <a href="{{ route('admin.bookings.index', ['filter' => 'upcoming']) }}" class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-semibold {{ request('filter') === 'upcoming' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' }}">
                        {{ __('common.Upcoming') }}
                    </a>
                    <a href="{{ route('admin.bookings.index', ['filter' => 'past']) }}" class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-semibold {{ request('filter') === 'past' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' }}">
                        {{ __('common.Past') }}
                    </a>
                    <a href="{{ route('admin.bookings.index', ['filter' => 'pending']) }}" class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-semibold {{ request('filter') === 'pending' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' }}">
                        {{ __('common.Pending') }}
                    </a>
                </div>
            </div>
                        ...
                        <div class="p-4">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-1">{{ $booking->subject->name }}</h3>
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mb-2">{{ $booking->student->name }} {{ __('common.with') }} {{ $booking->teacher->user->name }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ $booking->start_at->format('M j, Y g:i A') }}</p>
                                </div>
                            ...
                            <a href="{{ route('admin.bookings.show', $booking) }}" class="block w-full text-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition">
                                {{ __('common.View') }}
                            </a>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.Subject') }}</th>
                                    <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.Date & Time') }}</th>
                                ...
                                @forelse($bookings as $booking)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                        <td class="px-4 sm:px-6 py-3 sm:py-4">
                                            <div class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $booking->student->name }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 truncate md:hidden">{{ $booking->teacher->user->name }}</div>
                                        </td>
                                        ...
                                        <td class="px-4 sm:px-6 py-3 sm:py-4">
                                            <div class="text-sm text-gray-900 dark:text-white truncate">{{ $booking->subject->name }}</div>
                                        </td>
                                        <td class="px-4 sm:px-6 py-3 sm:py-4">
                                        ...
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 text-right text-sm font-medium">
                                            <a href="{{ route('admin.bookings.show', $booking) }}" class="inline-flex items-center justify-center px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-semibold rounded-lg transition">
                                                {{ __('common.View') }}
                                            </a>
                                        </td>
                                    </tr>
