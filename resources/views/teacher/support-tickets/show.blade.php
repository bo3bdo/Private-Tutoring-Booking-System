        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    @if($supportTicket->category)
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">{{ __('common.Category') }}</p>
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('common.' . $supportTicket->category) }}</p>
                        </div>
                    @endif
                    @if($supportTicket->assignedTo)
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $supportTicket->assignedTo->name }}</p>
                        </div>
                    @endif
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    @foreach($supportTicket->replies as $reply)
                        <div class="p-4 rounded-xl {{ $reply->user_id === auth()->id() ? 'bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/50' : 'bg-slate-50 dark:bg-gray-700/50 border border-slate-200 dark:border-gray-600' }}">
            ...
            @if(!$supportTicket->isClosed())
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 p-6">
                    ...
                    <form method="POST" action="{{ route('teacher.support-tickets.reply', $supportTicket) }}">
                        @csrf
                        <div class="mb-4">
                            <textarea name="message" rows="4" required class="w-full rounded-xl border-2 border-slate-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:border-emerald-400 dark:focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-400 dark:focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition resize-none" placeholder="{{ __('common.Type your reply...') }}"></textarea>
                        </div>
