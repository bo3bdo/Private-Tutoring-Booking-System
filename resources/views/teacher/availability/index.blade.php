    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('common.Add Availability') }}</h3>
                    <form method="POST" action="{{ route('teacher.availability.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('common.Start Time') }}</label>
                            <input type="time" name="start_time" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('common.End Time') }}</label>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('common.Current Availability') }}</h3>
                    <div class="space-y-3">
                        @forelse($availabilities as $availability)
                            <div class="flex items-center justify-between p-4 border border-slate-200 rounded-lg">
                                <div>
                                    <p class="font-semibold">{{ [__('common.Sunday'), __('common.Monday'), __('common.Tuesday'), __('common.Wednesday'), __('common.Thursday'), __('common.Friday'), __('common.Saturday')][$availability->weekday] }}</p>
                        ...
                        @empty
                            <p class="text-gray-500 text-center py-8">{{ __('common.No availability set. Add your available times above.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
