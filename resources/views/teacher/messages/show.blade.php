<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <a href="{{ route('teacher.messages.index') }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 overflow-hidden">
                    ...
                    @foreach($messages as $message)
                        <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                            <div class="max-w-md">
                                ...
                                <div class="p-4 rounded-2xl {{ $message->sender_id === auth()->id() ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-gray-900 dark:text-white' }}">
                                    <p class="text-sm whitespace-pre-wrap">{{ $message->body }}</p>
                                    @if($message->attachments->isNotEmpty())
                                        <div class="mt-3 space-y-2">
                                            @foreach($message->attachments as $attachment)
                                                <a href="{{ route('teacher.message-attachments.download', $attachment) }}" target="_blank" class="flex items-center gap-2 text-sm {{ $message->sender_id === auth()->id() ? 'text-blue-100 hover:text-white' : 'text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300' }}">
                ...
                <div class="p-6 border-t border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                    <form method="POST" action="{{ route('teacher.messages.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <textarea name="body" rows="3" required placeholder="{{ __('common.Type your message...') }}" class="w-full rounded-xl border-2 border-slate-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:border-blue-400 dark:focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition resize-none"></textarea>
                        </div>
                        <div class="mb-4">
                        ...
                        <button type="submit" class="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-600 rounded-xl text-sm font-semibold text-white shadow-lg hover:from-blue-700 hover:to-blue-800 dark:hover:from-blue-600 dark:hover:to-blue-500 transition">
