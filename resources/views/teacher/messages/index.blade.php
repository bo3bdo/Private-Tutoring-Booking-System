    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">{{ __('common.Chat with your students') }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($conversations->isEmpty())
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-12 text-center">
                    ...
                    <p class="text-gray-600 text-lg">{{ __('common.No conversations yet') }}</p>
                </div>
            @else
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
                    <div class="divide-y divide-slate-200">
                        @foreach($conversations as $conversation)
                            ...
                            @endphp
                            <a href="{{ route('teacher.messages.show', $conversation) }}" class="block p-6 hover:bg-slate-50 transition">
