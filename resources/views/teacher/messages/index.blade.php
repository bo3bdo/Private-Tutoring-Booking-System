    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('common.Chat with your students') }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($conversations->isEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 p-12 text-center">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-lg">{{ __('common.No conversations yet') }}</p>
                </div>
            @else
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-gray-700 overflow-hidden">
                    <div class="divide-y divide-slate-200 dark:divide-gray-700">
                        @foreach($conversations as $conversation)
                            ...
                            @endphp
                            <a href="{{ route('teacher.messages.show', $conversation) }}" class="block p-6 hover:bg-slate-50 dark:hover:bg-gray-700 transition">
                                <div class="flex items-start gap-4">
                                    <div class="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md">
                                    ...
                                    @if($unreadCount > 0)
                                        <div class="flex-shrink-0">
                                            <span class="inline-flex items-center justify-center w-6 h-6 bg-blue-600 dark:bg-blue-500 text-white text-xs font-bold rounded-full">
                                                {{ $unreadCount }}
                                            </span>
                                        </div>
                                    @endif
