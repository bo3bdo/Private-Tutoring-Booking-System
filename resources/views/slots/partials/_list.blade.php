    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
        <div class="p-6">
            <div class="mt-6 flex items-center justify-between">
                ...
                <span class="text-xs text-slate-500">{{ $daySlots->count() }} slots</span>
            </div>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <div class="mt-1 text-xs text-slate-500">
                                    Duration: {{ $slot->start_at->diffInMinutes($slot->end_at) }} minutes
                                </div>
                                @if(isset($isTeacher) && $isTeacher)
                                    <div class="mt-2">
                                        ...
                                        @if($slot->booking)
                                            <span class="ml-2 text-xs text-slate-600">Student: {{ $slot->booking->student->name }}</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
