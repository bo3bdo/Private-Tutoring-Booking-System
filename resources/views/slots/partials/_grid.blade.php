                                ...
                                @if(isset($isTeacher) && $isTeacher)
                                    ...
                                    @elseif($slot->status->value === 'blocked')
                                        <div class="rounded-xl border border-rose-200 bg-rose-50 px-2 py-2">
                                            <span class="inline-flex items-center rounded-full bg-rose-100 px-2 py-0.5 text-[11px] font-semibold text-rose-700">Blocked</span>
                                    ...
                                    @elseif($slot->status->value === 'booked')
                                        <div class="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2">
                                            <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-700">Booked</span>
                                            @if($slot->booking)
                                                <div class="mt-1 text-[10px] text-slate-600">{{ $slot->booking->student->name }}</div>
                                            @endif
                                        </div>
                                    @endif
                                @else
    ...
    @if($slots->isEmpty())
        <div class="p-8 text-center">
            <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8">
                <div class="text-sm font-semibold text-slate-800">No available slots</div>
                <div class="mt-1 text-xs text-slate-500">Try changing the week or teacher.</div>
            </div>
        </div>
    @endif
</div>
